...
The result? Within a week, the _Contract_ retention label was applied to all documents with the content type of _Contract document_ on any SharePoint sites the retention label was published to.
...
The result? Within a week, the _Hardware_ retention label was applied to all documents with a choice value of 'Hardware' on the _ContractType_ metadata column on any SharePoint sites the retention label was published to.
...
The result? Within a week, the _Software_ retention label was applied to all documents with a content type of _Contract document_ and a choice value of 'Software' on the _ContractType_ metadata column on any SharePoint sites the retention label was published to.
...
The result? Within a week, the _Expired Contract_ retention label was applied to all documents when a date either equal to today or in the past has been entered into the _DateExpired_ metadata column on any SharePoint sites the retention label was published to. Note: if a date isn't entered in the column OR a future date is entered in the column, a retention label is not applied.
